...
fn test_resolve_target_point_from_point() {
    ...
    let result = resolve_target_point(&decision, field.width().get::<meter>(), field.grid_dimensions());
...
fn test_resolve_target_point_from_region() {
    ...
    let result = resolve_target_point(&decision, field.width().get::<meter>(), grid_dims);
...
fn test_resolve_target_point_grid_cell_center_consistent_with_region() {
    ...
    let point_from_cell = resolve_target_point(&decision_cell, field.width().get::<meter>(), grid_dims).unwrap();
    let point_from_region = resolve_target_point(&decision_region, field.width().get::<meter>(), grid_dims).unwrap();
