...
fn test_field_x_is_width_z_is_length() {
    ...
    assert!(
        (w - 68.0).abs() < 0.1,
        "Field width must be ~68 m (X axis), got {w}"
    );
    assert!(
        (l - 104.615).abs() < 0.1,
        "Field length must be ~104.6 m (Z axis), got {l}"
    );
}
...
fn test_half_a_zone_spans_z_axis() {
    ...
    assert!(
        ...
        "half_a min_x must be 0, got {}",
        rect.min.x.get::<meter>()
    );
    assert!(
        (rect.max.x.get::<meter>() - w).abs() < 0.1,
        "half_a max_x must be field_width={w:.2}, got {}",
        rect.max.x.get::<meter>()
    );
    assert!(
        (rect.min.z.get::<meter>() - 0.0).abs() < 0.1,
        "half_a min_z must be 0 (Team A goal end), got {}",
...
fn test_penalty_area_a_is_wide_not_deep() {
    ...
    assert!(
        ...
        "penalty_area_a must start at Z=0, got {}",
        rect.min.z.get::<meter>()
    );
}
...
fn test_center_circle_is_at_field_center() {
    ...
    assert!(
        (cx - w / 2.0).abs() < 0.5,
        "center_circle X must be field_width/2={:.2}, got {cx:.2}",
        w / 2.0
    );
    assert!(
        (cz - l / 2.0).abs() < 0.5,
        "center_circle Z must be field_length/2={:.2}, got {cz:.2}",
        l / 2.0
    );
}
...
fn test_field_zone_covers_entire_field() {
    ...
    assert!(
        ...
        "field zone max_x must equal width={w:.2}, got {}",
        rect.max.x.get::<meter>()
    );
    assert!(
        (rect.max.z.get::<meter>() - l).abs() < 0.01,
        "field zone max_z must equal length={l:.2}, got {}",
        rect.max.z.get::<meter>()
    );
}
...
fn test_halves_are_adjacent_and_cover_full_length() {
    ...
    let rect_b = match &field
        .get_zone("half", Some(ynwa_core::team::Team::B))
        .unwrap()
        .geometry
    {
        ZoneGeometry::Rectangle(r) => r.clone(),
    ...
    assert!(
        ...
        rect_a.max.z.get::<meter>(),
        rect_b.min.z.get::<meter>()
    );
    assert!(
        (rect_a.min.z.get::<meter>() - 0.0).abs() < 0.01,
        "half_a must start at Z=0"
    );
    assert!(
...
fn test_cell_z40_is_in_half_b_not_half_a() {
    ...
    let region = field
        .grid_dimensions()
        .create_region(
            GridCell::from_notation("Z40").unwrap(),
            GridCell::from_notation("Z40").unwrap(),
        )
        .unwrap();
    let center = region.center(field.grid_dimensions(), w);
    ...
    assert!(
        point_in_rect(cx, cz, &half_b),
        "Cell Z40 center ({cx:.4}, {cz:.4}) must lie inside half_b"
    );
    assert!(
        !point_in_rect(cx, cz, &half_a),
        "Cell Z40 center ({cx:.4}, {cz:.4}) must NOT lie inside half_a"
    );
}
...
fn test_cell_m40_is_in_penalty_area_b() {
    ...
    let region = field
        .grid_dimensions()
        .create_region(
            GridCell::from_notation("M40").unwrap(),
            GridCell::from_notation("M40").unwrap(),
        )
        .unwrap();
    let center = region.center(field.grid_dimensions(), w);
