...
#[test]
fn test_find_nearest_opponent() {
    // Test finding nearest opponent
    let test_script = r#"
function test_find_nearest_opponent()
    local result = find_nearest_opponent()
    
    -- Should return a table with opponent and distance fields
    if type(result) ~= "table" then
        error("find_nearest_opponent should return a table")
    end
    
    if result.opponent == nil and result.distance == nil then
        error("Result should have opponent and distance fields")
    end
    
    if result.distance ~= nil and type(result.distance) ~= "number" then
        error("Distance should be a number")
    end
end
...
"#;
    ...
    assert!(
        player_state.last_error.is_none(),
        "find_nearest_opponent test failed: {:?}",
        player_state.last_error
    );
}

#[test]
fn test_am_i_closest_teammate_to_ball() {
    // Test checking if player is closest to ball
    let test_script = r#"
function test_closest_to_ball()
    local result = am_i_closest_teammate_to_ball()
    
    -- Should return boolean
    if type(result) ~= "boolean" then
        error("am_i_closest_teammate_to_ball should return boolean")
    end
end
...
"#;
    ...
    assert!(
        player_state.last_error.is_none(),
        "am_i_closest_teammate_to_ball test failed: {:?}",
        player_state.last_error
    );
}
